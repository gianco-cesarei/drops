...
export function DiscoveryEnvironment({ items }: { items: DiscoveryItem[] }) {
  ...
  return <div className="environment-layout"><aside className="environment-rail"><span className="rail-label">Categorie</span><Categories types={state.types} onChange={(types) => state.update(types)} label="Categorie Grid" /></aside><div className="environment-content"><form className="catalog-search" role="search" onSubmit={submit}><label htmlFor="catalog-query">Ricerca</label><div><input id="catalog-query" value={draft} onChange={(event) => setDraft(event.target.value)} placeholder="Cerca label, artisti, set, storie…" /><button>Cerca</button></div></form><p className="result-summary"><strong>{visible.length}</strong> contenuti · development fixture</p><div className="discovery-grid">{visible.map((item) => <DiscoveryCard key={item.id} item={item} />)}</div></div></div>
}
